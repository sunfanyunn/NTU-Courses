...
//thread pool here
//=====================================
void upd_pid(csiebox_server *server) {
  fprintf(stderr, "upd_pid to %d\n", getpid());
  char path[ PATH_MAX ];
  sprintf(path, "%s/csiebox_server.pid", server->run_path);
  int fd = open(path, O_WRONLY | O_CREAT | O_TRUNC);
  char buf[10]; sprintf(buf, "%d", getpid());
...
int csiebox_server_run(csiebox_server* server) {
  ...
  while (1) {
	while( select( max_fd+1, &read_set, NULL, NULL, NULL) == -1 )
        if( errno == EINTR ) 
...
static int parse_arg(csiebox_server* server, int argc, char** argv) {
  ...
  while ((keylen = getdelim(&key, &keysize, '=', file) - 1) > 0) {
    ...
    if (strcmp("path", key) == 0) {
      ...
      strncpy(server->run_path, val, vallen);
      accept_config[2] = 1;
    }
  }
  free(key);
...
static void logout(csiebox_server* server, int conn_fd) {
  fprintf(stderr, "Logout, conn_fd = %d\n", conn_fd);
  free(server->client[conn_fd]);
